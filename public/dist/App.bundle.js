...
function autocomplete(input, latInput, lngInput) {
  ...
  input.on('keydown', function (e) {
    if (e.keyCode === 15) e.preventDefault();
  });
};
...
function ajaxHeart(e) {
  ...
  axios.post(this.action).then(function (res) {
    ...
    if (isHearted) {
      _this.heart.classList.add('heart__button--float'); // add animated when the buttun became hearted
      setTimeout(function () {
        return _this.heart.classList.remmove('heart__button--float');
      }, 2500); // remove the class after 2,5s to avoid having invisible hearts
    }
  }).catch(console.error);
};
...
function loadPlaces(map) {
  ...
  // it's possible to get the current one of the user with : nagiator.geolocation.getCurrentPosition
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = dompurify.sanitze('<div class="search__result">No results for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    ...
    var current = search.querySelector('.' + activeClass); // search the current active selector
    var items = search.querySelectorAll('.search__result');
    var next = void 0;
    if (e.keyCode === 40 && current) {
      // 40 keyCode = arrow down
      ...
      // if we press up, then go to the previous one
...
var heartForms = (0, _bling.$$)('form.heart'); // $$ = listen to all
